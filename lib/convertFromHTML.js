  ...
  while (child) {
    ...
    if (sibling && fragmentBlockTags.indexOf(nodeName) >= 0 && inBlock) {
      ...
      if (newBlockInfo !== false) {
        ...
        var shouldAddEmptyChunk = options.flatNestedDivs && text.trim() === '' && text.includes('\n');
        var anotherChunk = shouldAddEmptyChunk ? getEmptyChunk() : getSoftNewlineChunk(newBlockType, depth, options.flat, newBlockData);
